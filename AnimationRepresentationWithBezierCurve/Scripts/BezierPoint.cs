...
public class BezierPoint : MonoBehaviour{
	...
	public Vector3 handle1
	{
		...
		{ 
            ...
            if (value.x == 0) _handle1.x = 1e-5f ; else _handle1.x = value.x;
            if (value.y == 0) _handle1.y = 1e-5f; else _handle1.y = value.y;
            if (value.z == 0) _handle1.z = 1e-5f; else _handle1.z = value.z;
	...
	/// <summary>
	/// 	- Weight of controler point
	/// </summary>

	[Range(0, 1)]
	[SerializeField] private float _outWeight;
	public float outWeight { get { return _outWeight; } set { _outWeight = value; } }
	...
	public Vector3 handle2
	{
		...
		{ 
			...
			if (value.x == 0) _handle2.x = 1e-5f; else _handle2.x = value.x;
			if (value.y == 0) _handle2.y = 1e-5f; else _handle2.y = value.y;
            if (value.z == 0) _handle2.z = 1e-5f; else _handle2.z = value.z;
            /*if(handleStyle == HandleStyle.None) handleStyle = HandleStyle.Broken;
			else*/
    ...
    [Range(0, 1)]
    [SerializeField] private float _inWeight;
    public float inWeight { get { return _inWeight; } set { _inWeight = value; } }
